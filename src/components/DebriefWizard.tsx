...
export default function DebriefWizard({
  ...
  const inputCls =
    "bg-[#16161c] text-white border border-white/15 rounded px-3 h-11 focus:outline-none focus:border-electric-blue font-mono text-center text-base transition-colors";
  const labelCls =
      ...
      return (
        <>
          <div className="px-5 -mt-1 mb-1">
            <p className="text-[11px] font-condensed font-bold text-neutral-300 bg-white/5 border-l-2 px-3 py-2"
  ...
  return createPortal(
    <div className="fixed inset-0 bg-pure-black/95 backdrop-blur-sm flex items-center justify-center z-[300] p-4">
      <motion.div
        ...
        className="border-2 border-white/15 bg-[#0a0a0e] w-full max-w-lg max-h-[92vh] overflow-y-auto shadow-[0_0_40px_rgba(0,0,0,0.9)]"
      >
        {/* HEADER */}
        <div className="sticky top-0 z-10 bg-[#0a0a0e] border-b border-white/10 px-5 py-3.5 flex items-center justify-between">
        ...
        {/* FOOTER */}
        <div className="sticky bottom-0 bg-[#0a0a0e] border-t border-white/10 px-5 py-3.5 flex items-center gap-2">
          <button onClick={goPrev} disabled={current === 0}
            className="flex items-center gap-1 px-3 h-11 rounded font-mono text-xs font-bold uppercase tracking-wider border border-white/15 text-neutral-300 hover:bg-white/5 disabled:opacity-30 disabled:cursor-not-allowed transition-all cursor-pointer">
