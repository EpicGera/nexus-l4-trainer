  ...
  return (
    <SectionCard title="Matriz de sustitución" subtitle="Equivalencias aeróbicas y de gimnasia (Mayhem · cap. 45B)">
      <div className="flex flex-wrap items-end gap-3 mb-3">
        <Field label="Distancia (m)">
          <input
            ...
            className="w-28 bg-black/60 rounded-sm h-[38px] px-3 text-white font-mono text-sm focus:outline-none focus:border-electric-blue"
          />
        </Field>
        <Field label="En">
          <select
            ...
            className="bg-black/60 rounded-sm h-[38px] px-3 text-white font-mono text-sm focus:outline-none focus:border-electric-blue"
          >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
        {others.map((m) => (
          <div key={m} className="rounded-sm bg-black/40 p-2 text-center">
            ...
            <div className="text-base font-brutalist text-white">
              ...
              <span className="text-[9px] text-[color:var(--color-label)]"> m</span>
            </div>
          </div>
        ))}
      ...
      <p className="mt-2 text-[9px] font-mono text-[color:var(--color-label)] uppercase">{SHUTTLE_NOTE}</p>

      <div className="mt-4 space-y-2">
        <div className={TXT.label}>Sustituciones de gimnasia</div>
        {GYM_SUBSTITUTIONS.map((g) => (
          <div key={g.movement} className="bg-black/40 rounded-sm px-3 py-2">
