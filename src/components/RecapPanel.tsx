...
export default function RecapPanel({ variant, week, onClose }: Props) {
  ...
  const ModalBar = ({ m, pct }: { m: string; pct: number }) => (
    <div className="mb-1.5 last:mb-0">
      ...
      <div className="h-1.5 bg-white/5"><div className="h-full bg-white" style={{ width: `${pct}%` }} /></div>
    </div>
  );

  return (
    <div className="w-full border border-white/15 bg-[#0A0A0A] rounded-none mb-6 relative">
      {onClose && (
      ...
      <div id={nodeId} className="p-6 bg-[#0A0A0A]">
            ...
            <div className="space-y-1.5 mb-5">
              {(() => { const max = Math.max(1, ...data.month!.weeks.map((w) => w.tonnageKg)); return data.month!.weeks.map((w) => (
                <div key={w.week}>
                  ...
                  <div className="h-2 bg-white/5"><div className={`h-full ${w.weekNumber === 4 ? "bg-signal-red" : "bg-white"}`} style={{ width: `${(w.tonnageKg / max) * 100}%` }} /></div>
                </div>
              )); })()}
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
        ...
        <div className="mt-5 pt-3 border-t border-white/10 text-[9px] font-mono text-neutral-600 uppercase tracking-widest text-center">
