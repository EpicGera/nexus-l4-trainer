...
export default function RecapPanel({ variant, week, onClose }: Props) {
  ...
  const Stat = ({ label, value }: { label: string; value: React.ReactNode }) => (
    <div className="p-3 text-center">
      <div className="text-[9px] font-mono text-[color:var(--color-label)] uppercase tracking-widest mb-1">{label}</div>
  ...
  return (
    <div className="w-full bg-[color:var(--color-card)] rounded-none mb-6 relative">
      {onClose && (
        <button onClick={onClose} className="absolute top-2 right-2 text-[color:var(--color-label)] hover:text-white p-1 cursor-pointer z-10" title="Cerrar recap">
          <X size={16} />
        </button>
      )}
      ...
      <div id={nodeId} className="p-6 bg-[color:var(--color-card)]">
        <div className="border-b border-[color:var(--color-line)] pb-3 mb-4">
          <div className="text-[10px] font-mono text-[color:var(--color-label)] uppercase tracking-[0.3em]">NEXUS L4 · RESUMEN</div>
        ...
        {variant === "week" && data.wk && (
          <>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mb-5">
              ...
              <Stat label="Trabajo" value={<>{data.wk.workKJ}<span className="text-[10px] text-[color:var(--color-label)] ml-1">kJ</span></>} />
              <Stat label="RPE prom" value={data.wk.avgRpe ?? "—"} />
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              <div>
                <div className="text-[10px] font-mono font-bold text-[color:var(--color-label)] uppercase tracking-widest mb-2">Balance modal</div>
              ...
              <div>
                ...
                )) : <div className="text-[11px] font-mono text-[color:var(--color-label)]">Sin registros</div>}
              </div>
            </div>
        ...
        {variant === "month" && data.month && (
          <>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mb-5">
              ...
              <Stat label="Trabajo" value={<>{data.month.totalWorkKJ}<span className="text-[10px] text-[color:var(--color-label)] ml-1">kJ</span></>} />
              <Stat label="RPE prom" value={data.month.avgRpe ?? "—"} />
            </div>
            {/* Evolución semanal: barras de tonelaje + RPE */}
            <div className="text-[10px] font-mono font-bold text-[color:var(--color-label)] uppercase tracking-widest mb-2">Evolución semanal (tonelaje)</div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              <div>
                <div className="text-[10px] font-mono font-bold text-[color:var(--color-label)] uppercase tracking-widest mb-2">Balance modal del mes</div>
        ...
        <div className="mt-5 pt-3 border-t border-[color:var(--color-line)] text-[9px] font-mono text-[color:var(--color-label)] uppercase tracking-widest text-center">
      ...
      <div className="flex gap-2 p-4 pt-0">
        <button onClick={() => run(exportRecapPng, "png")} disabled={busy}
          className="flex-1 flex items-center justify-center gap-2 py-2.5 text-white font-brutalist text-[11px] tracking-widest uppercase hover:bg-white hover:text-black transition-all cursor-pointer disabled:opacity-40">
