        ...
        <div className="flex gap-1.5">
          {MODALITIES.map((m) => (
            <button
              ...
              className={`flex-1 py-2 rounded-sm border text-[11px] font-mono uppercase cursor-pointer ${modality === m ? "bg-electric-blue text-black border-transparent" : "bg-black/40 text-neutral-300 border-white/15"}`}
            >
              {MODALITY_ES[m]}
            </button>
          ))}
      ...
      <div className="mt-3">
        <Field label="Patrón">
          <select
            ...
            className="w-full bg-black/60 border border-white/15 rounded-sm h-[38px] px-3 text-white font-mono text-sm focus:outline-none focus:border-electric-blue"
          >
            {PATTERNS.map((p) => (
      ...
      <div className="flex flex-wrap items-center gap-2">
        {([
          ...
          <button
            ...
            className={`text-[10px] font-mono font-black uppercase tracking-wider px-3 py-1.5 rounded-sm cursor-pointer transition-all ${
              scope.kind === k ? "bg-electric-blue text-black" : "bg-white/5 text-neutral-300 hover:bg-white/10"
            }`}
          >
            {lbl}
          </button>
        ...
        {scope.kind !== "global" && chapters.length > 1 && (
          <select
            ...
            className="bg-black/60 border border-white/15 rounded-sm h-[30px] px-2 text-white font-mono text-[11px] focus:outline-none focus:border-electric-blue"
          >
            {chapters.map((c) => (
          ...
          <div className="grid grid-cols-4 gap-1.5">
            {(Object.keys(coverage.timeDomain) as BlockTimeDomain[]).map((td) => {
              ...
              return (
                <div
                  key={td}
                  className="rounded-sm border border-white/10 p-2 text-center"
        ...
        <SectionCard title="Sin clasificar" subtitle="Clasificá estos movimientos para el desglose modal/skills completo">
          <div className="space-y-1.5">
            {data.unclassified.map((name) => (
              <div key={name} className="flex items-center justify-between gap-2 bg-black/40 border border-white/10 rounded-sm px-3 py-2">
                  ...
                  {TIME_DOMAINS.map((td) => {
                    ...
                    return (
                      <td key={td} className="p-1">
                        <div
                          className="h-9 rounded-sm border border-white/10 flex items-center justify-center text-[9px] font-mono"
        ...
        <SectionCard title="Agrupación registrada" subtitle="Pacing de tus metcons de intervalos sellados (cap. 43)">
          <div className="space-y-2">
            {data.intervalGroupings.map((g, i) => {
              ...
              return (
                <div key={`${g.dayId}-${i}`} className="flex items-center justify-between gap-2 bg-black/40 border border-white/10 rounded-sm px-3 py-2">
