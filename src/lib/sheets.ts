...
export async function exportToGoogleSheets(accessToken: string, athleteLogs: any[]): Promise<string> {
  ...
  // Export the live per-user program (cache → clean starter), so each user's
  // own plan lands in their own newly-created sheet — never personal data.
  const database = loadCachedWorkouts() || getDefaultProgram();
  const weeks = Object.keys(database);
